...
class c_net(nn.Module):
    def __init__(self, depth=[1, 1, 1, 2]):
        ...
        self.Encoder = nn.ModuleList([
            ...
            nn.Sequential(*[RB(base_channel) for _ in range(1)]),
            Down_scale(base_channel),
            BasicConv(base_channel * 2, base_channel * 2, 3, 1),
            nn.Sequential(*[RB(base_channel * 2) for _ in range(1)]),
            Down_scale(base_channel * 2),
            BasicConv(base_channel * 4, base_channel * 4, 3, 1),
            nn.Sequential(*[RB(base_channel * 4) for _ in range(1)]),
            Down_scale(base_channel * 4),
        ])
        # Middle
        self.middle = nn.Sequential(*[RB(base_channel * 8) for _ in range(2)])
        # decoder
    ...
    def forward(self, x):
        x = self.conv_first(x)  # conv+norm+gelu
